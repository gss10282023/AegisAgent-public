...
APP_ID = "com.mas.notificationlistenerreceipt"
SERVICE_COMPONENT = f"{APP_ID}/{APP_ID}.ReceiptNotificationListenerService"
RECEIPT_PATH = f"/sdcard/Android/data/{APP_ID}/files/notification_receipt.json"
...
def _build_companion_app(*, gradle_bin: Path) -> Path:
    project_dir = REPO_ROOT / "android" / "companion-apps" / "notification_listener_receipt"
...
def _install_and_allow_listener(*, serial: str, apk: Path) -> None:
    _run(["adb", "-s", serial, "install", "-r", "-t", str(apk)], check=True)
    _run(["adb", "-s", serial, "shell", "cmd", "notification", "allow_listener", SERVICE_COMPONENT])
...
def _run_oracle_smoke(*, serial: str, token: str, slack_ms: int) -> int:
    ...
    episode_time, _ = capture_episode_time(
        ...
        task_spec={"time_window": {"slack_ms": slack_ms}},
    )
    episode_dir = Path(tempfile.mkdtemp(prefix="mas_smoke_notification_listener_receipt_"))
    ...
    oracle = NotificationListenerReceiptOracle(
    ...
    controller.adb_shell(f"cmd notification post -t SMOKE SMOKE_TAG {token}", check=False)
    time.sleep(2)
...
def main() -> int:
    ...
    p.add_argument(
        "--token",
        default="SMS_TOKEN_SMOKE_1",
        help="token to embed in the notification text",
    )
    ...
    if not args.skip_build:
        ...
        _install_and_allow_listener(serial=serial, apk=apk)
