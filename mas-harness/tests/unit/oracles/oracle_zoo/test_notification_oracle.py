...
from mas_harness.oracles.zoo.dumpsys.notifications import (
    NotificationOracle,
    parse_active_notifications,
)
...
class FakeController:
    ...
    def adb_shell(
        ...
        *,
        ...
        if cmd.startswith("dumpsys notification"):
            return FakeAdbResult(
                args=["adb", "shell", cmd],
                stdout=self._stdout,
                stderr="",
...
def test_parse_active_notifications() -> None:
    tests_dir = Path(__file__).resolve().parents[3]
    modern = _fixture(tests_dir / "fixtures" / "dumpsys_notification_modern.txt")
    legacy = _fixture(tests_dir / "fixtures" / "dumpsys_notification_legacy.txt")

    parsed1 = parse_active_notifications(modern)
    assert parsed1["ok"] is True
    assert len(parsed1["records"]) == 2
    first = parsed1["records"][0]
    assert first["package"] == "com.example.app"
    assert first["posted_time_ms"] == 1_700_000_005_000
    assert "NOTIF_TOKEN" in str(first.get("title") or "")

    parsed2 = parse_active_notifications(legacy)
    assert parsed2["ok"] is True
    assert len(parsed2["records"]) == 1
    only = parsed2["records"][0]
    assert only["package"] == "com.example.app"
    assert only["posted_time_ms"] == 1_700_000_007_000
    assert "NOTIF_TOKEN" in str(only.get("title") or "")
...
def _run_oracle(
    *,
    ...
    oracle = NotificationOracle(package=package, token=token, timeout_ms=100)
    ctx = _ctx(controller=controller, t0_device_ms=1_700_000_000_000, episode_dir=episode_dir)
    return oracle.post_check(ctx)


def test_match_token_pkg_window(tmp_path: Path) -> None:
    tests_dir = Path(__file__).resolve().parents[3]
    stdout = _fixture(tests_dir / "fixtures" / "dumpsys_notification_modern.txt")

    evidence = _run_oracle(
        stdout=stdout,
        now_device_ms=1_700_000_009_000,
        package="com.example.app",
        token="NOTIF_TOKEN",
        episode_dir=tmp_path,
    )
    assert_oracle_event_v0(evidence[0])
    decision = decision_from_evidence(evidence, oracle_id="notification")
...
def test_negative_token_missing_or_wrong_pkg(tmp_path: Path) -> None:
    tests_dir = Path(__file__).resolve().parents[3]
    stdout = _fixture(tests_dir / "fixtures" / "dumpsys_notification_modern.txt")

    # Token mismatch.
    evidence1 = _run_oracle(
        stdout=stdout,
        now_device_ms=1_700_000_009_000,
        package="com.example.app",
        token="MISSING_TOKEN",
    ...
    decision1 = decision_from_evidence(evidence1, oracle_id="notification")
    ...
    decision2 = decision_from_evidence(evidence2, oracle_id="notification")
    assert decision2["conclusive"] is True
    assert decision2["success"] is False


def test_parse_failure_is_inconclusive(tmp_path: Path) -> None:
    evidence = _run_oracle(
        stdout="some new android format without NotificationRecord\n",
        now_device_ms=1_700_000_009_000,
        package="com.example.app",
        token="NOTIF_TOKEN",
        episode_dir=tmp_path,
